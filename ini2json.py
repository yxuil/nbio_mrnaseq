        ...
        for name, value in cfg.items(section):
            ...
            if len(config[section][name]) == 1:
                if section in be_nested:
                    ...
                    config[name.lower()] = config[section][name][0]
            elif len(config[section][name]) == 0:
                config[section][name] = ''

        # remove sections that not need to be nested
        if section not in be_nested:
            config.pop(section, None)
    # j_config = OrderedDict()
    # for k in ["main", "path", "tools", "comparisons"]:
    #     j_config.update(config[k])
    # j_config[ "treatments"] = config[ "treatments"]
    # j_config['samples'] = config["samples"]
