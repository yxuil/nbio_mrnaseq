...
<Session genome="{{ genome }}" locus="chr1:1-200" version="4">
    <Resources>
      {% for sample in samples %}
        <Resource path="../alignment/{{ sample }}.genome.sorted.bam"/>
      {% endfor %}
    </Resources>
    ...
    <Panel height="{{ panel_height }}" name="panel_{{ sample }}" width="{{ panel_width }}">
        ...
        id="../alignment/{{ sample }}.genome.sorted.bam_coverage" 
        ...
        id="../alignment/{{ sample }}.genome.sorted.bam" 
...
if __name__ == '__main__':
    USAGE = "Usage: {} genome sample1[,sample2,...] output".format(sys.argv[0])
    if len(sys.argv) < 4:
        print USAGE
        exit()
    else:
        make_IGV_session(sys.argv[1], sys.argv[2].split(','), sys.argv[3])
